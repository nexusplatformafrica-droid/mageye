...
For Vercel, the portfolio must use Nuxt static generation and publish `.vercel/output/static`; do not point Vercel at the old copied `dist/public` directory or use the Node server preset.
...
**How to apply:** Keep the frontend-only build command as `nuxt generate`, use the artifact’s `.vercel/output/static` as the Vercel output directory, and keep admin edits browser-local unless shared publishing is explicitly requested.